...
BEGIN
  ...
  FROM items WHERE category = 'motor' AND owner_id = (auth.user_id())::uuid;
...
BEGIN
  SELECT COALESCE(MAX(substring(reference_code from '\d+$')::int), 0) + 1 INTO next_seq
  FROM items WHERE category = 'kit' AND owner_id = (auth.user_id())::uuid;
...
BEGIN
  SELECT COALESCE(MAX(substring(reference_code from '\d+$')::int), 0) + 1 INTO next_seq
  FROM items WHERE category = 'plane' AND owner_id = (auth.user_id())::uuid;
...
BEGIN
  SELECT COALESCE(MAX(substring(reference_code from '\d+$')::int), 0) + 1 INTO next_seq
  FROM items WHERE category = 'part' AND owner_id = (auth.user_id())::uuid;
...
BEGIN
  SELECT COALESCE(MAX(substring(reference_code from '\d+$')::int), 0) + 1 INTO next_seq
  FROM items WHERE category = 'print' AND owner_id = (auth.user_id())::uuid;
...
BEGIN
  SELECT COALESCE(MAX(substring(reference_code from '\d+$')::int), 0) + 1 INTO next_seq
  FROM items WHERE category = 'other' AND owner_id = (auth.user_id())::uuid;
